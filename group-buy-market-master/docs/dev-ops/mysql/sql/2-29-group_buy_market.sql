...
-- ===== DEMO 数据动态修正（导入时执行，确保前端直接可见拼团） =====
SET @now = NOW();

-- 活动时间与状态：生效中，未来 30 天有效
...
-- 拼团队伍：全部置为“进行中”，有效期 +1 天，预留空位
UPDATE group_buy_order
   SET valid_start_time = @now,
       valid_end_time   = DATE_ADD(@now, INTERVAL 1 DAY),
       status = 0,
       complete_count = 0,
       lock_count = 1,
       update_time = @now;

-- 拼团成员：有效期 +1 天，状态置为进行中
...
-- 额外保证至少三支队伍一定有空位
UPDATE group_buy_order SET lock_count = 1 WHERE team_id IN ('15721600','21762498','29487599');
