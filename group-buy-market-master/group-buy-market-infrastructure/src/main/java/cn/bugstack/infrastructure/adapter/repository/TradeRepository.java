...
public class TradeRepository implements ITradeRepository {
    ...
    public NotifyTaskEntity settlementMarketPayOrder(GroupBuyTeamSettlementAggregate groupBuyTeamSettlementAggregate) {
        ...
        // 允许返回0（订单已完成，重复订单复用）或1（订单首次完成）
        // 0的情况：用户取消订单后重新参团，复用了旧的已支付订单记录
        // 1的情况：正常首次支付完成
        if (updateOrderListStatusCount > 1) {
            // 理论上不应该更新多条记录，如果出现说明数据异常
            throw new AppException(ResponseCode.UPDATE_ZERO);
        }
        
        // 2. 更新拼团达成数量（无论是否重复订单，都需要增加complete_count）
        // 注意：只有updateOrderListStatusCount==1时才真正增加，否则会重复计数
        int updateAddCount = 0;
        if (updateOrderListStatusCount == 1) {
            updateAddCount = groupBuyOrderDao.updateAddCompleteCount(groupBuyTeamEntity.getTeamId());
        } else {
            // 订单已完成，说明是复用旧订单，不重复增加计数，但需要继续检查团队是否满员
            updateAddCount = 1; // 设置为1，让后续流程继续
        }
        if (1 != updateAddCount) {
            throw new AppException(ResponseCode.UPDATE_ZERO);
        }
